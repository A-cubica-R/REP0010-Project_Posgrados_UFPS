...
public class JornadaRestController {
    ...
    @GetMapping("/list/{id}")
    public ResponseEntity<JornadaOutput> findById(@PathVariable int id) {
        JornadaOutput output = processor.findById(id);
        if (output != null) {
    ...
    @PostMapping("/create")
    public ResponseEntity<JornadaOutput> create(@RequestBody CREATE request) {
        JornadaOutput output = processor.create(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(output);
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<JornadaOutput> update(@PathVariable int id, @RequestBody UPDATE request) {
        try {
            JornadaOutput updated = processor.update(id, request);
